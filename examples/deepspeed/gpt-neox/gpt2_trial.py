...
class GPT2Trial(DeepSpeedTrial):
    def __init__(self, context: DeepSpeedTrialContext) -> None:
        ...
        self.neox_args.build_tokenizer()  # tokenizer needs to be build in training in order to set the padding vocab
        ...
        self.mpu = self.context.wrap_mpu(DeepSpeedMPU(model.mpu))
    ...
    def train_batch(self, data_iterator, epoch_idx, batch_idx):
        if self.neox_args.is_pipe_parallel:
            ...
            for _ in range(self.neox_args.gradient_accumulation_steps):
                ...
                losses.append(loss)
            reduced_loss = {"lm_loss": np.mean(losses)}
        ...
        if (
            ...
            megatron_utils.print_rank_0(
                "rank: {} | time: {} | exiting the program at iteration {}".format(
                    rank, time_str, self.neox_args.iteration
                )
            )
